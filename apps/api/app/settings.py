...
class Settings(BaseSettings):
    ...
    # Optional fallback password scripts/seed.py hashes onto a staff row
    # that doesn't have one yet (unset here, so a fresh clone/CI database
    # never gets a real credential). Never overwrites a password_hash
    # that's already set — see the seed() docstring.
    ...
    # POST /auth/staff/password/reset: unlike every other WhatsApp message
    # in this product, there is no human at a front desk to tap send for a
    ...
    staff_password_reset_cooldown_minutes: int = 5
